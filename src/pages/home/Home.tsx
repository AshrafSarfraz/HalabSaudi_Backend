...
import { getBrands } from '../../backend/Api/brandApi';
...
const Home = () => {
  ...
  useEffect(() => {
    const fetchRestaurants = async () => {
      ...
      try {
        // ⬇️ yaha direct fetch nahi, sirf helper function
        const restaurantsList = await getBrands();

        const groupedByCountry = restaurantsList.reduce(
          (acc: any, restaurant: any) => {
            const country = restaurant.selectedCountry || 'Unknown';
            if (!acc[country]) {
              acc[country] = [];
            }
          ...
          <>
            {Object.keys(restaurantsByCountry).map((country, index) => (
              <div key={index} className="mt-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-6 gap-6 mt-4">
                  {restaurantsByCountry[country].map(
                    (restaurant: any, idx: number) => (
                      <div
                        key={restaurant._id || idx}
                        className="w-full bg-white rounded-2xl shadow-lg border-opacity-50 border-black px-4 pb-3 pt-2"
                      >
                        <img
                          src={restaurant.img}
                        ...
                        <p className="text-gray-600 text-sm">
                          {restaurant.selectedVenue || 'N/A'}{' '}
                          {restaurant.selectedCountry}
                        </p>
                ...
                {restaurantsByCountry[country].length === 0 && (
                  <p className="text-gray-500 mt-4">
                    No restaurants available for {country}.
                  </p>
                )}
              </div>
            ))}
...
//                     <div
//                       key={index}
//                       className="w-full bg-white rounded-2xl shadow-lg border-opacity-50 border-black px-4"
//                     >
//                       <img
//                         src={restaurant.img}
//                         alt={restaurant.nameEng}
//                         className="w-full h-20 object-cover rounded mb-2"
//                       />
//                       <h3 className="text-lg font-bold text-gray-800">{restaurant.nameEng}</h3>
