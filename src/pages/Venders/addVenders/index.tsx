...
import VenderAccount from "../../../component/modal/venderAccount/VenderAccount";
...
const VenderAccounts: React.FC = () => {
  ...
  useEffect(() => {
    setLoading(true);
    const q = query(collection(fireDB, "H-Vender_Account"), orderBy("time"));
  ...
  const handleDeleteAccount = async (id: string, email: string) => {
    ...
    try {
      await deleteDoc(doc(fireDB, "H-Vender_Account", id));
  ...
  return (
    <Layout>
      <div className="container mx-auto py-6 px-4">
        <h1 className="text-3xl font-bold mb-6 text-gray-800">Vender Account Management</h1>
        ...
        <div className="flex flex-col md:flex-row md:justify-between items-center mb-6 gap-3">
          ...
          <button
            onClick={() => handlePress()}
            className="bg-red-600 hover:bg-red-700 text-white px-5 py-2 rounded-md transition duration-200 shadow-md"
          >
            Vender Account
          </button>
        </div>
      ...
      <VenderAccount isOpen={isModalOpen} onClose={closeModal} editData={editData} />
    </Layout>
  );
};

export default VenderAccounts;
